...
void BackgroundRenderer::copyColors(BackgroundCell** bg) {
	...
	for (unsigned int x = 0; x < WorldState::WORLD_WIDTH; x++) {
		for (unsigned int y = 0; y < WorldState::WORLD_WIDTH; y++) {
			unsigned int vertIndex = (x + y * (WorldState::WORLD_WIDTH + 1)) * vertWidth;
	...
	unsigned int verticesSize = sizeof(float) * (WorldState::WORLD_WIDTH + 1) * (WorldState::WORLD_WIDTH + 1) * vertWidth;
...
void BackgroundRenderer::initRenderData() {
	...
	vertices = new float[(WorldState::WORLD_WIDTH + 1) * (WorldState::WORLD_WIDTH + 1) * vertWidth];
	for (unsigned int x = 0; x < WorldState::WORLD_WIDTH + 1; x++) {
		for (unsigned int y = 0; y < WorldState::WORLD_WIDTH + 1; y++) {
			unsigned int index = (x + y * (WorldState::WORLD_WIDTH + 1)) * vertWidth;
			vertices[index] = x;
			vertices[index + 1] = y;
		}
	}
	...
	indices = new unsigned int[WorldState::WORLD_WIDTH * WorldState::WORLD_WIDTH * quadWidth];
	for (unsigned int x = 0; x < WorldState::WORLD_WIDTH; x++) {
		for (unsigned int y = 0; y < WorldState::WORLD_WIDTH; y++) {
			unsigned int topRight = (x + 1) + y * (WorldState::WORLD_WIDTH + 1);
			unsigned int bottomRight = (x + 1) + (y + 1) * (WorldState::WORLD_WIDTH + 1);
			unsigned int bottomLeft = x + (y + 1) * (WorldState::WORLD_WIDTH + 1);
			unsigned int topLeft = x + y * (WorldState::WORLD_WIDTH + 1);
			
			unsigned int index = (x + y * WorldState::WORLD_WIDTH) * quadWidth;
	...
	unsigned int indicesSize = sizeof(unsigned int) * WorldState::WORLD_WIDTH * WorldState::WORLD_WIDTH * quadWidth;
